...
mod wizard;
use wizard::{WizardUI, WizardEvent};
...
fn main() -> winrt::Result<()> {
    ...
    let mut wizard = WizardUI::new(window, desktop_source, event_loop.create_proxy())?;
    ...
    event_loop.run(move |event, _, control_flow| {
        *control_flow = ControlFlow::Wait;
        match event {
            ...
            Event::UserEvent(WizardEvent::GoToStep2) => {
                if let Err(err) = wizard.go_to_step2() {
                    eprintln!("{:?}", err);
                    *control_flow = ControlFlow::Exit
                }
            }
            Event::UserEvent(WizardEvent::UsbDeviceFound(name)) => {
                if let Err(err) = wizard.add_usb_device(&name) {
                    eprintln!("{:?}", err);
                    *control_flow = ControlFlow::Exit
                }
            }
            Event::UserEvent(WizardEvent::GoToStep3) => {
                if let Err(err) = wizard.go_to_step3() {
                    eprintln!("{:?}", err);
                    *control_flow = ControlFlow::Exit
                }
            }
            Event::UserEvent(WizardEvent::SetProgress(cur, total)) => {
                if let Err(err) = wizard.set_progress(cur, total) {
                    eprintln!("{:?}", err);
                    *control_flow = ControlFlow::Exit
                }
            }
